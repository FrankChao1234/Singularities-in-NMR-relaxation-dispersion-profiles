...
Wa = 0
Wb = 0/float(const)
kexab = (100)/float(const)
kexbc = (10**(2+1))/float(const)
kexac = 0
pa = 0.80
pb = 0.01
pc = 0.19
...
for j in range(40):
  for i in range(40):
    ...
    trans_relax = math.log(math.sqrt(coor_ya**2+coor_xa**2)/float(pa))/float(-2*0.001*cyc*tcp)

    print (1000/float(4*tcp), 40*j, trans_relax)
  print ( )
